...
| 小项目 extension asset-db runtime 触发情况必须被处理或证明 | Runtime request capture | 若 runtime 实际触发 `ViewStateGroup` extension request，则基于 package / AssetDB / frozen metadata / engine request fact 处理；若未触发，记录 `not-triggered-in-small-project`；通用 enable/disable/global config 语义不作为当前门槛。 | 后续 browser / HTTP capture 证据 | `missing` |
| import/native/pack/redirect route 均有 fact-backed contract 或未触发证明 | Engine-source probe + HTTP contract | import 已有 production bundle-config-backed contract；native 已用真实 engine-captured `ImageAsset` URL 证明 production route，只在 bundle config 映射 UUID 为 `cc.ImageAsset` 时放行，未配置但存在的 native file 返回 404；pack、redirect 必须由 engine source 和 bundle config 事实触发或记录未触发证明；frozen `library` 角色：`compatibility baseline`，不是 URL mapping 权威。`remote` 当前不作为 Task 1 完成门槛。 | `vitests/suites/runtime-preview/http-url-capture.probe.test.ts`、`vitests/suites/runtime-preview/launcher-runtime-preview.test.ts`、`vitests/suites/runtime-preview/http-contract.test.ts`、`docs/dev/runtime-preview-architecture-facts-20260606.md` | `partial` |
