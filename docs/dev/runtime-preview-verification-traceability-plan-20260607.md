...
- Modify only if Task 8A found a factual root page or approved diagnostic route: `src/runtime-preview/server/runtime-preview-routes.ts`
...
- Task 8A identifies either a factual root preview page / preview-app entry, or an explicit diagnostic harness route.
...
- If a preview-app entry is copied or adapted from old editor / backup code, the test must inspect it for forbidden overrides and URL/base/route mapping logic before browser smoke can run.
- If only a diagnostic route exists, the browser smoke result is `partial` and cannot be used to claim full runtime preview scene/resource completion.
...
- Reuse: `vitests/shared/browser-runtime-smoke.ts` after Task 8B
...
`small-project-cli-integration.test.ts` must call this helper and then reuse the browser smoke helper from Task 8B. If Task 8B is still blocked by missing preview page facts, this test must stay skipped or `blocked-by-fact-gap` with a clear reason in the acceptance document; it must not open a browser and pass by waiting for no immediate error.
...
下一步不应直接补 Task 8B 浏览器 smoke。Task 1/2/3 已完成，当前执行顺序必须从仍未闭环的事实缺口继续：

1. Task 4：继续 CLI/editor output consistency，明确 `library/cli`、engine `editor/library`、frozen editor `library` / `temp/programming` 的差异哪些应修生成链，哪些是可接受的 source-backed split layout。
2. Task 5 Step 2/3/4：补 pack、redirect 和小项目 extension runtime trigger 事实。pack/redirect 可以构造最小 fixture，但必须由 engine source、CLI output 或真实 generated artifact 证明，不能手写近似 URL 或 bundle config。
3. Task 8A：先确认 root preview page / preview-app entry 的事实来源和 ready signal 归属。如果没有事实入口，Task 8B 保持 `blocked-by-fact-gap`。
4. Task 8B：只有在 Task 8A 完成后，才实现 browser smoke harness；如果只是 diagnostic harness，验收状态只能是 `partial`。
5. Task 9：复用 Task 8B harness，启动真实 CLI child process，对小项目 `E:\own_space\cocos_work_lab_38x` 做集成验收并产出证据。

原因：

- 矩阵、ledger 和 full-suite 稳定性已经建立，继续重复 Task 1/2/3 会浪费执行窗口。
- 当前最容易偏离的是用浏览器 harness 自己定义业务事实，所以必须先完成 preview entry fact ledger。
- 当前剩余实现风险主要在真实 CLI/editor output、pack/redirect/extension fact 和 browser entry source，不在旧的 full-suite timeout。
