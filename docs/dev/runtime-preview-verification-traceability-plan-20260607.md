...
- Browser smoke remains blocked until Task 8B adds production root `/` and `/preview-app/*`, and Task 8C proves required routes.

- [x] **Step 4: Add entry contract test**

`browser-entry-contract.test.ts` should verify:

- the absence of `/` and `/preview-app/*` is recorded as current implementation gap, not as desired contract.
...
- `vitests/suites/runtime-preview/browser-entry-contract.test.ts` currently verifies `/` and `/preview-app/index.js` return `404` in current route behavior, `/settings.js` remains active, and old editor / backup entry facts exist. This is now treated as historical implementation-gap evidence; Task 8B must invert the entry assertions.
...
- `Creator 3.8.6 preview-app source + CLI adaptation` 已经由备份分支文档和源码证明是 runtime preview 的核心入口输入。当前 `/` 和 `/preview-app/*` 404 是必须修复的 implementation gap。
...
- [ ] **Step 1: 修正 browser entry contract test**
...
- [ ] **Step 2: 迁入 preview-app 源码，不手改编译产物**
...
- [ ] **Step 3: 接入 preview-app build script**
...
- [ ] **Step 4: 接入 root template 和 `/preview-app/*` route**
...
- [ ] **Step 5: 验证 browser entry HTTP contract**
...
下一步不应直接补 browser smoke。Task 1/2/3 已完成，当前执行顺序必须把 Creator preview-app production entry 接回主线：

1. Task 8B：接入 Creator 3.8.6 preview-app 源码、构建脚本、root `/` 和 `/preview-app/*` production entry。这是当前最高优先级。
2. Task 8C：按 preview-app/template 实际请求补 `/scripting/engine/*`、polyfills、scene、missing-asset、preview-error、socket/no-op socket 等 required routes。
3. Task 4 / Task 5：继续保持 CLI/editor output consistency、pack/redirect/extension runtime trigger 事实闭环；这些不能阻塞 preview-app entry 接入，但会影响 browser smoke 完成度。
4. Task 8D：基于 production root `/` 执行真实 browser smoke，监听 console/pageerror/network，等待 ready 和稳定窗口。
5. Task 9：复用 Task 8D harness，启动真实 CLI child process，对小项目 `E:\own_space\cocos_work_lab_38x` 做集成验收并产出证据。
