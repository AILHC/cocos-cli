...
- 2026-06-24 shared project library 默认化后，production 默认 `projectLibraryRoot` 已回到 `<project>/library`，CLI project records 改为 `library/.cli-assets-*` sidecar 文件；上面的 `library/cli` 决策只代表 2026-06-09 的 isolated output 阶段，不再代表当前默认策略。
- 2026-06-26 已补充 scene route 回归：`/scene-list` 和 `/scene/<uuid>.json` 在 shared output 下优先读取 `library/.cli-assets-data.json`，并避免 stale `library/cli/.assets-data.json` 遮蔽当前 sidecar records。
