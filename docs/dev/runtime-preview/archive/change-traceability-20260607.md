...
当前已有 CLI-only runtime preview path 的 active 证据。engine-source test harness 是 candidate：它证明当前测试方向和常量语义，但还没有形成 CLI behavior 闭环。`ec7f8d2161` Node.js PAL engine patch 仍为 `needs-review`：它是当前事实链的重要输入，但还没有精确到某个 CLI commit / CLI behavior / CLI test 的完整闭环，不能提前标为 active。
