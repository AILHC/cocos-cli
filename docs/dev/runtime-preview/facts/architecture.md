...
- CLI AssetDB 当前 production 默认 project db library 是 `<project>/library`，project records 使用 shared sidecar 文件 `<project>/library/.cli-assets-data.json`、`<project>/library/.cli-assets-info.json`、`<project>/library/.cli-assets-dependency.json` 和 `<project>/library/.cli-assets`；legacy isolated output `<project>/library/cli/.assets-data.json` 仅作为兼容路径。internal db library 是 `<engine>/editor/library`；extension mount 仍注册到 `<project>/library/cli-extensions/<name>`。冻结 editor reference library 是 editor 产物 `<project>/library`，只能作为对照产物，不能直接等同当前 CLI shared sidecar records。
...
- 历史 CLI output 诊断曾记录 `library/cli/.assets-data.json`、`.assets-info.json`、`.assets-dependency.json` 存在；2026-06-24 shared project library 默认化后，production 默认 project records 改为 `library/.cli-assets-data.json`、`.cli-assets-info.json`、`.cli-assets-dependency.json` 和 `.cli-assets`，legacy `library/cli/.assets-data.json` 不能再作为默认 production 判据。`src/core/assets/asset-config.ts` 将 `internal` 的 library 指向 engine `editor/library`，该目录存在 `.internal-data.json`、`.internal-info.json`、`.internal-dependency.json`，并包含 frozen `.internal-data.json` 中代表性的 TTF 与 primitive native-like sample；代表性 project asset 的 `url/depends/versionCode` 与 frozen editor project library 一致，代表性 internal asset 的 `url/depends` 与 frozen editor project library 一致，但 primitive root UUID `1263d74c-8167-4928-91a6-4e2672411f47` 的 `versionCode` 为 frozen `1`、engine internal `3`；frozen editor project library 使用 `info1.0.0` 命名，当前 `@cocos/asset-db` sidecar records 使用 `.cli-assets-*`，因此 output consistency 需区分 frozen Editor records、shared CLI sidecar records 和 legacy isolated records；`temp/cli/programming/packer-driver/targets/preview/import-map.json` 存在。
...
- `settings.engine.builtinAssets` 必须与 browser 中实际加载的 engine feature 集合使用同一选择边界。普通 preview 加载完整 engine，必须收集全部 feature 的 dependent assets；runtime preview 的 bare `cc` 映射到项目 `includeModules` 生成的 `cce:/internal/x/cc`，因此只收集该 feature graph 的 dependent assets。default physics material `ba21476f-2866-4f81-9c4d-6e359316e448` 仅在 feature graph 包含 3D physics 时加入：需要它却遗漏会触发 `PhysicsSystem.initDefaultMaterial()` 加载失败，不需要它却加入会因 `cc.PhysicMaterial` 未注册而反序列化失败。
