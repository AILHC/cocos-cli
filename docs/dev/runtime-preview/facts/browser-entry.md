...
旧 Editor preview server 参考源码已迁入 `docs/dev/reference/old_editor_preview_server/`，作为长期 historical reference。后续 runtime preview 排查应引用该仓库内路径；不要再依赖外部 tmp repo 路径作为长期事实入口。
