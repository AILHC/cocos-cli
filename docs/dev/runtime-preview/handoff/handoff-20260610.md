...
> **Status:** superseded historical handoff. 本文保留 2026-06-10 当时交接状态，不代表当前执行入口。当前状态以 [../issues.md](../issues.md)、[../README.md](../README.md) 和 [../acceptance/matrix.md](../acceptance/matrix.md) 为准。
...
- `docs/dev/runtime-preview/facts/cli-output-and-meta-side-effects.md`
...
- `docs/dev/runtime-preview/archive/verification-traceability-20260607.md`
  - 验证和可追溯计划。
- `docs/dev/runtime-preview/archive/preview-app-route-inventory-20260608.md`
...
- `libraryRoot` 为 `D:\ps_copy\p6\trunk\Project\GameClient\feature-c\library\cli`。
...
```text
D:\ps_copy\p6\trunk\Project\GameClient\feature-c\assets\product\animation\a_bdmx_hit.anim.meta
...
- source `.meta` 写回必须与 Editor 3.8.6 保持一致；`library/cli`、`temp/cli` 生成和 source `.meta` 写入必须分开统计。
...
本节保留 2026-06-10 交接时的建议，只作为历史上下文。当前入口以 `docs/dev/runtime-preview/README.md`、`docs/dev/runtime-preview/issues.md` 和 `docs/dev/README.md` 为准。

1. 先读 `AGENTS.md` 和本文件。
2. 再读 `docs/dev/runtime-preview/acceptance/feedback-20260609.md` 的 6.7、6.8。
