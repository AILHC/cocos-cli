...
| RP-ISSUE-016 | 小项目真实 CLI child process 集成验收不稳定 | `open` | 2026-06-12 focused run 中 `browser-runtime-smoke.test.ts` 通过，但 `small-project-cli-integration.test.ts` 在 120s 内未等到 `preview:ready`，失败停在 `builder:init:start` 后；需要单独核对 timeout、builder init 耗时和是否真实 hang。 | [acceptance/matrix.md](acceptance/matrix.md)、[facts/browser-entry.md](facts/browser-entry.md) | 无当前计划 | `vitests/suites/runtime-preview/small-project-cli-integration.test.ts` |
