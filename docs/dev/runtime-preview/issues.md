...
| RP-ISSUE-007 | 浏览器脚本加载是否可以并发加速 | `deferred` | 当前只记录问题，不执行并发化修改；不能在未确认 Cocos module loading 语义前改加载顺序。正式说明见 [../modules/scripting.md](../modules/scripting.md)。 | [facts/browser-loading-and-cache-20260611.md](facts/browser-loading-and-cache-20260611.md)、[../facts/scripting-generated-modules.md](../facts/scripting-generated-modules.md) | 无当前计划 | 后续单独计划 |
...
| RP-ISSUE-019 | runtime preview root 页面未按 Editor preview 业务语义使用项目 `preview-template`，默认设备尺寸固定为 960x640 | `open` | 当前 CLI root `/` 固定渲染 CLI 内置 `static/runtime-preview/index.ejs`，未优先使用 `<project>/preview-template/index.ejs`；`Default` device 固定 fallback 960x640，未从项目 `settings.screen.designResolution` 派生。目标是贴近 Editor browser preview 默认逻辑，不通过 CLI 专用 opt-in 参数改变行为。 | [facts/browser-entry.md](facts/browser-entry.md)、[plans/ready-url-and-logging-contract-20260612.md](plans/ready-url-and-logging-contract-20260612.md) | [plans/ready-url-and-logging-contract-20260612.md](plans/ready-url-and-logging-contract-20260612.md) | `vitests/suites/runtime-preview/browser-entry-contract.test.ts`、`launcher-runtime-preview.test.ts` |
