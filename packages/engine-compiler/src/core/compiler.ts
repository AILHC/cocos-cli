...
export class EngineCompiler {
    ...
    async compile(force: boolean = false): Promise<void> {
        ...
        await this.fixCurrentImportMapExtensions();
    ...
    private async fixCurrentImportMapExtensions(): Promise<boolean> {
        const outputDir = this.isWeb ? ps.join(this.outDir, 'web') : ps.join(this.outDir, 'editor');
        return fixImportMapExtensions(ps.join(outputDir, 'import-map.json'), this.enginePath);
    }
