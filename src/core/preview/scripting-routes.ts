...
import { pathExists, stat, readFile, realpath } from 'fs-extra';
...
async function resolveAllowedFileRoots(): Promise<string[]> {
    const { Engine } = await import('../engine');
    const info: any = Engine.getInfo();
    const roots = [GlobalPaths.enginePath, info?.native?.path, info?.typescript?.path];

    try {
        const { default: assetConfig } = await import('../assets/asset-config');
        roots.push(...assetConfig.data.assetDBList.map(({ library }) => library));
    } catch {
        // AssetDB may not be initialized while the shared routes are being registered.
    }

    const canonicalRoots = await Promise.all(roots
    ...
    return [...new Set(canonicalRoots.filter((root): root is string => root !== null))];
}
    ...
    },
    {
        ...
        async handler(req: Request, res: Response, next: NextFunction) {
            try {
                ...
                const allowedRoots = await resolveAllowedFileRoots();
                const allowed = allowedRoots.some((root) => isPathWithinRoot(resolved, root));
                if (!allowed) {
