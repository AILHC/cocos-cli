...
import { installCCONBJsonDownloadFallback } from '../../engine/cconb-json-download-fallback';
...
export async function startup(options: {
    ...
    installCCONBJsonDownloadFallback(cc.assetManager, { isBrowser: true, serverURL });
    ...
    async function loadNativeAsset(asset: any, uuid: string): Promise<void> {
        ...
        if (!res.ok) {
            throw new Error(`Native asset fetch failed (${res.status}): ${uuid}`);
        }
    ...
    async function loadFromServer(uuid: string, onComplete: any) {
        try {
            ...
            let ext = 'json';
            try {
                const extRes = await fetch(`${serverURL}/scene/query-extname/${encodedUuid}`);
            ...
            try {
                ...
                // Width/data-sensitive leaf assets publish readiness together with
                // the partial cache entry. Failed owners reject waiters instead of
                // handing out an invalid partial instance.
                ...
                if (needsReady) {
                    ...
                    nativeReady.set(uuid, readyPromise);
                }
                    ...
                    if (uuidList && uuidList.length > 0) {
                        ...
                        await Promise.all(
                            uuidList
                                ...
                                .map((depUuid: string) => new Promise<void>((resolve, reject) => {
                                    am.loadAny(depUuid, (err: any, depAsset: any) => {
                                        if (err) {
                                            reject(err);
                                            return;
                                        }
                                        if (depAsset) depMap[depUuid] = depAsset;
                                        resolve();
                                    });
                                })),
                        );
                ...
                if (asset && am.assets.get(uuid) === asset) {
                    am.assets.remove(uuid);
                }
                rejectReady?.(error);
                throw error;
            } finally {
                if (details && !detailsReturned) {
                    Details.pool?.put?.(details);
                }
                if (readyPromise && nativeReady.get(uuid) === readyPromise) {
                    nativeReady.delete(uuid);
                }
                if (resolveReady && am.assets.get(uuid) === asset) {
                    resolveReady();
                }
            }
