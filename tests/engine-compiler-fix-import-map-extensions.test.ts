...
import { EngineCompiler } from '../packages/engine-compiler/src/core/compiler';
import { fixImportMapExtensions } from '../packages/engine-compiler/src/core/import-map-utils';

jest.mock('@cocos/quick-compiler', () => ({ QuickCompiler: class QuickCompiler {} }));
jest.mock('@cocos/ccbuild', () => ({ StatsQuery: { create: jest.fn() } }));
jest.mock('@cocos/lib-programming/dist/utils', () => ({ editorBrowserslistQuery: [] }));
...
describe('engine-compiler / fixImportMapExtensions', () => {
    ...
    it('VERSION=3 的 web warm cache 跳过 rebuild 时仍补全 import-map 扩展名', async () => {
        const webImportMapFile = ps.join(engineRoot, 'bin', '.cache', 'dev-cli', 'web', 'import-map.json');
        await fsExtra.outputFile(ps.join(engineRoot, 'pal/system-info/web/system-info.ts'), '// web ts');
        await fsExtra.outputFile(ps.join(engineRoot, 'bin', '.cache', 'dev-cli', 'VERSION'), '3');
        await fsExtra.outputJson(webImportMapFile, {
            imports: {
                'pal/system-info': 'q-bundled:///fs/pal/system-info/web/system-info',
            },
        });

        const compiler = EngineCompiler.create(engineRoot, true);
        (compiler as any).statsQuery = {};
        jest.spyOn(compiler, 'generateCompiler').mockResolvedValue({} as any);
        const rebuild = jest.spyOn(compiler, 'rebuild').mockResolvedValue();

        await compiler.compile(false);

        expect(rebuild).not.toHaveBeenCalled();
        await expect(fsExtra.readJson(webImportMapFile)).resolves.toMatchObject({
            imports: {
